...
const App: React.FC = () => {
  ...
  const handleGenerate = useCallback(async (params: GenerateVideoParams) => {
    ...
    try {
        ...
        // Enhanced Error Detection for API Key / Billing issues
        const lowerMsg = errorMessage.toLowerCase();
        
        if (lowerMsg.includes('api_key_invalid') || lowerMsg.includes('invalid api key') || lowerMsg.includes('key not found')) {
            errorMessage = "API ключ недействителен. Пожалуйста, выберите корректный ключ.";
            isAuthError = true;
        } else if (lowerMsg.includes('api_key_expired') || lowerMsg.includes('expired')) {
             errorMessage = "Срок действия API ключа истек. Пожалуйста, обновите ключ.";
             isAuthError = true;
        } else if (lowerMsg.includes('billing') || lowerMsg.includes('project not linked')) {
             errorMessage = "Проект требует привязки биллинг-аккаунта (Veo требует оплаты).";
             isAuthError = true;
        } else if (lowerMsg.includes('403') || lowerMsg.includes('permission_denied')) {
            errorMessage = "Доступ запрещен (403). Проверьте права API ключа.";
            isAuthError = true;
        } else if (lowerMsg.includes('429') || lowerMsg.includes('quota') || lowerMsg.includes('resource_exhausted')) {
             errorMessage = "Превышен лимит квот API. Попробуйте позже.";
        } else if (lowerMsg.includes('400') || lowerMsg.includes('invalid argument')) {
             errorMessage = "Ошибка запроса (400). Проверьте параметры генерации.";
        }

        setFeed(prev => prev.map(p => p.id === id ? { ...p, status: PostStatus.ERROR, errorMessage } : p));
        showToast(errorMessage, 'error');
        logEvent('error', 'Generation failed', { error: e.message, code: isAuthError ? 'AUTH_ERROR' : 'GENERIC' });
  ...
  const handleUpgrade = useCallback(async (post: FeedPost) => {
    ...
    try {
        ...
        if (lowerMsg.includes('api_key_invalid') || lowerMsg.includes('invalid api key')) {
             ...
             errorMessage = "Ошибка биллинга.";
             isAuthError = true;
        } else if (lowerMsg.includes('403') || lowerMsg.includes('permission_denied')) {
             errorMessage = "Доступ запрещен.";
             isAuthError = true;
        } else if (lowerMsg.includes('429') || lowerMsg.includes('quota')) {
             errorMessage = "Лимит квот исчерпан.";
        }
        ...
        setFeed(prev => prev.map(p => p.id === post.id ? { ...p, status: PostStatus.SUCCESS } : p)); // Revert to success (old video) but show error
        showToast(errorMessage, "error");
    }
  }, []);

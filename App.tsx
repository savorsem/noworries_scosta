...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const hasBundledApiKey = Boolean(import.meta.env.VITE_GEMINI_API_KEY);

    const checkKeySelection = async () => {
      if (window.aistudio) {
        const hasKey = await window.aistudio.hasSelectedApiKey();
        if (!hasKey && !hasBundledApiKey) {
          setShowKeyDialog(true);
        }
      } else if (!hasBundledApiKey) {
        setShowKeyDialog(true);
      }
    };
  ...
  return (
    <div
      className={`h-[100dvh] w-screen flex flex-col overflow-hidden font-sans selection:bg-white/10 ${theme}`}
      data-theme={theme}
      style={{ backgroundColor: 'var(--theme-bg)' }}
    >
      <AnimatePresence>
        {showKeyDialog && <ApiKeyDialog onContinue={() => setShowKeyDialog(false)} />}
      </AnimatePresence>
      <SettingsDrawer
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -40 }}
            className="fixed top-0 left-1/2 -translate-x-1/2 z-[300] px-6 py-4 rounded-3xl bg-neutral-900 border border-white/10 backdrop-blur-3xl shadow-2xl flex items-center gap-4"
          >
            {toast.type === 'error' ? (
        ...
        <header className="sticky top-0 z-40 w-full px-6 py-5 flex items-center justify-between bg-black/50 backdrop-blur-2xl border-b border-white/5 transition-colors duration-300">
          <div className="flex items-center gap-5">
            <button
              ...
              className="p-2.5 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 transition-all text-white"
            >
              <Menu size={18} />
            </button>
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="flex items-center gap-2.5 px-6 py-2.5 rounded-full bg-white text-black hover:scale-105 active:scale-95 transition-all shadow-xl shadow-white/10"
            >
              <Sparkles size={16} />
              <span className="text-xs font-black uppercase tracking-tight">В студию</span>
            </button>
          </div>
        </header>

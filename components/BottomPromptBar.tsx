...
import { AspectRatio, GenerationMode, Resolution, VedoModel } from '../types';
...
export default function BottomPromptBar({ onGenerate, isGenerating = false, onOpenSettings }: BottomPromptBarProps) {
  ...
  const [model, setModel] = useState<VedoModel>(VedoModel.VEO_FAST);
            ...
            <div className="flex items-end gap-2">
              <div className="flex-1">
                <textarea
                  ...
                  rows={1}
                  className="w-full resize-none rounded-2xl bg-white/5 px-4 py-3 text-sm sm:text-base text-white placeholder:text-white/40 outline-none focus:ring-2 focus:ring-indigo-500/40 h-12 sm:h-12"
                  onKeyDown={(e) => {

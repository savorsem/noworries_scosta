...
const getActiveKey = () => (import.meta.env.VITE_GEMINI_API_KEY as string | undefined) || '';
...
export const enhancePrompt = async (currentPrompt: string): Promise<string> => {
  ...
  const response = await ai.models.generateContent({
    ...
    Твоя задача: взять короткую идею промпта-инженер и превратить её в детальный, богатый кинематографический промпт на русском.

    Включи описание:
    1. Освещение (Golden hour, cinematic lighting, neon, etc.)
    ...
    Верни ТОЛЬКО улучшенный текст промпта без каких-либо кавычек и лишних слов.`,
  });
  ...
  const response = await ai.models.generateContent({
    ...
    contents: {
      parts: [
        { inlineData: { mimeType, data: base64 } },
        {
          text: "Проанализируй этот кадр из видео. Опиши окружение, угол камеры и действие персонажей. Верни детальный промпт на английском языке.",
        },
      ],
    },
  });
...
export const replaceCharacterInFrame = async (
  frameBase64: string,
  frameMime: string,
  charBase64: string,
  charMime: string
...
export const createDirectorSession = (settings?: StudioSettings, useThinking: boolean = false): Chat => {
  ...
  const instruction = `Вы — СТЭНЛ, ИИ-кинорежиссер.
...
export const generateImage = async (prompt: string): Promise<string> => {
  ...
  try {
    ...
    const part = res.candidates?.[0]?.content?.parts?.find(p => (p as any).inlineData);
    return (part as any)?.inlineData?.data || '';
  }
};
...
export const editImage = async (base64: string, mimeType: string, prompt: string): Promise<string> => {
  ...
  const response = await ai.models.generateContent({
    ...
    contents: {
      parts: [
        { inlineData: { mimeType, data: base64 } },
        { text: `Отредактируй это изображение: ${prompt}. Сохрани композицию.` },
      ],
    },
  });
  const part = response.candidates?.[0]?.content?.parts?.find(p => (p as any).inlineData);
...
export const generateCharacterReplacement = async (
